...
export default function App(){
  ...
  function openDoors(){ setDoorOpen(true) }
  function closeDoors(){ setDoorOpen(false) }
  ...
  function handleSelect(target){
    ...
    setTimeout(()=>{
      ...
      const t = target;
      const step = t > s ? 1 : -1;
      for(let f = s+step; ; f+=step){ steps.push(f); if(f===t) break }
      let idx = 0;
  ...
  function handleEnter(){ if(doorOpen){ closeDoors(); return } openDoors(); setDisplayFloor('FLOOR') }

  useEffect(()=>{
    function onKey(e){ if(e.key === 'Escape') closeDoors() }
    ...
    <div className="scene">
      <div className="elevator-shell">
        <div className="elevator-frame">
          <div className={"elevator-inner "+(moving? 'moving':'')}>
            ...
            <div className="panel" role="region" aria-label="Elevator panel">
              ...
              <div className="button-grid">
                {[1,2,3,4,5].map(n=>{
                  ...
                  return (<button key={n} className={"floor-btn "+(isActive? 'active':'')} aria-pressed={isActive} onClick={()=>handleSelect(n)} disabled={moving}>{n}</button>)
                })}
              </div>
            ...
            <div className="door-viewport">
              ...
              <div className={"content "+(doorOpen && currentFloor? 'visible':'') } style={{zIndex:5}}>
                {currentFloor===1 && (<div><h1>1F — ABOUT</h1><p>Park So Hyun · Graphic / Brand / Content Designer</p></div>)}
                {currentFloor===2 && (<div><h1>2F — EXPERIENCE</h1><p>Service & Personal brand experience highlights</p></div>)}
                {currentFloor===3 && (<div><h1>3F — TOOLS</h1><p>Photoshop · Figma · Illustrator · InDesign · AI Tools</p></div>)}
                {currentFloor===4 && (<div><h1>4F — PROJECTS</h1><p>KKOTPICK · noww · RE:ZUL — brief cards</p></div>)}
                {currentFloor===5 && (<div><h1>5F — LETTER</h1><p>How I found design · Strengths · Aspirations</p></div>)}
                {!currentFloor && doorOpen && (<div><h1>WELCOME</h1><p>Press a floor button to enter Park So Hyun's portfolio.</p></div>)}
              </div>
